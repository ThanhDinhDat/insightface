...
def main():
    ...
    if args.mode == 'image':
        ...
        for file_name in file_name_list:
            ...
            tic = time.time()
            bboxes, infer_time = face_predictor.predict(im, resize_scale=0.5, score_threshold=0.6, top_k=10000, \
                                            NMS_threshold=0.4, NMS_flag=True, skip_scale_branch_list=[])
            print("Duration: {}".format(time.time() - tic))
            for bbox in bboxes:
                print(bbox)
        ...
        for file_name in file_name_list:
            ...
            while cv2.waitKey(1) < 0:
                ...
                tic = time.time()
                bboxes, infer_time = face_predictor.predict(frame, resize_scale=0.5, score_threshold=0.7, top_k=10000, \
                                                NMS_threshold=0.3, NMS_flag=True, skip_scale_branch_list=[])
                toc = time.time()
                detect_time = (toc - tic) * 1000
                print("Duration: {}".format(detect_time))
                face_num = 0
                for bbox in bboxes:
                    face_num += 1
